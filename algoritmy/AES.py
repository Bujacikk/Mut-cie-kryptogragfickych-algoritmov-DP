...
class AES:
    ...
    def __init__(self, plaintext, key):
        self.AESalgorythm(plaintext, key)

    def AESalgorythm(self, plaintext, key):
        ...
        for byte in plaintext:
            temp.append(byte)
            if len(temp) == 16:
                Encrypted16Bytes = self.AES128Encryption(temp, key)
        ...
        for byte in encryptedMessage:
            temp.append(byte)
            if len(temp) == 16:
                Decrypted16Bytes = self.AES128Decryption(temp, key)
    ...
    def AES128Encryption(self, plaintextBytes, key):
        ...
        round = 0
        state = self.addRoundKey(state, keySchedule, round)
        ...
        state = self.subBytes(state, False)
        state = self.shiftRows(state, False)
        state = self.addRoundKey(state, keySchedule, round + 1)
    ...
    def AES128Decryption(self, ciphertextBytes, key):
        ...
        state = self.addRoundKey(state, keySchedule, noOfRounds)
    ...
    def subBytes(self, state, inverse):
    ...
    def shiftRows(self, state, inverse):
        ...
        if inverse == True:
            for i in range(1, noOfColumns):
                state[i] = self.rightShift(state[i], count)
                count += 1
        # Left shift for decryption
        else:
            for i in range(1, noOfColumns):
                state[i] = self.leftShift(state[i], count)
                count += 1
    ...
    def mixColumns(self, state, inverse):
        ...
        for i in range(noOfColumns):
            ...
            if inverse == True:
                s0 = self.multiplyBy0x0e(state[0][i]) ^ self.multiplyBy0x0b(state[1][i]) ^ self.multiplyBy0x0d(state[2][i]) ^ self.multiplyBy0x09(state[3][i])
                s1 = self.multiplyBy0x09(state[0][i]) ^ self.multiplyBy0x0e(state[1][i]) ^ self.multiplyBy0x0b(state[2][i]) ^ self.multiplyBy0x0d(state[3][i])
                s2 = self.multiplyBy0x0d(state[0][i]) ^ self.multiplyBy0x09(state[1][i]) ^ self.multiplyBy0x0e(state[2][i]) ^ self.multiplyBy0x0b(state[3][i])
                s3 = self.multiplyBy0x0b(state[0][i]) ^ self.multiplyBy0x0d(state[1][i]) ^ self.multiplyBy0x09(state[2][i]) ^ self.multiplyBy0x0e(state[3][i])
            # Matrix multiplication for encryption
            else:
                s0 = self.multiplyBy0x02(state[0][i]) ^ self.multiplyBy0x03(state[1][i]) ^ state[2][i] ^ state[3][i]
                s1 = state[0][i] ^ self.multiplyBy0x02(state[1][i]) ^ self.multiplyBy0x03(state[2][i]) ^ state[3][i]
                s2 = state[0][i] ^ state[1][i] ^ self.multiplyBy0x02(state[2][i]) ^ self.multiplyBy0x03(state[3][i])
                s3 = self.multiplyBy0x03(state[0][i]) ^ state[1][i] ^ state[2][i] ^ self.multiplyBy0x02(state[3][i])
    ...
    def addRoundKey(self, state, keySchedule, round):
    ...
    def multiplyBy0x02(self, number):
    ...
    def multiplyBy0x03(self, number):
        """Function to multiply by 0x03 in Galois Space."""
        # Breaking into simpler parts
        return (self.multiplyBy0x02(number) ^ number)

    def multiplyBy0x09(self, number):
        """Function to multiply by 0x09 in Galois Space."""
        # Breaking into simpler parts
        return self.multiplyBy0x02(self.multiplyBy0x02(self.multiplyBy0x02(number))) ^ number

    def multiplyBy0x0b(self, number):
        """Function to multiply by 0x0b in Galois Space."""
        # Breaking into simpler parts
        return self.multiplyBy0x02(self.multiplyBy0x02(self.multiplyBy0x02(number))) ^ self.multiplyBy0x02(number) ^ number

    def multiplyBy0x0d(self, number):
        """Function to multiply by 0x0d in Galois Space."""
        # Breaking into simpler parts
        return self.multiplyBy0x02(self.multiplyBy0x02(self.multiplyBy0x02(number))) ^ self.multiplyBy0x02(self.multiplyBy0x02(number)) ^ number

    def multiplyBy0x0e(self, number):
        """Function to multiply by 0x0e in Galois Space."""
        # Breaking into simpler parts
        return self.multiplyBy0x02(self.multiplyBy0x02(self.multiplyBy0x02(number))) ^ self.multiplyBy0x02(self.multiplyBy0x02(number)) ^ self.multiplyBy0x02(number)
