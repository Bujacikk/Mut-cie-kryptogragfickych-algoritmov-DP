...
class RSA:
    ...
    def __init__(self, bit_length, msg):
        self.RSAalgorythm(bit_length, msg)

    def RSAalgorythm(self, bit_length, msg):
        print("Running RSA...")
        print("Generating public/private keypair...")
        public, private = self.generate_keypair(p, q, 2 ** bit_length)  # 8 is the keysize (bit-length) value.
        print("Public Key: ", public)
        print("Private Key: ", private)

        print([ord(c) for c in msg])
        encrypted_msg = self.encrypt(msg, public)
        print("Encrypted msg: ")
        print(''.join(map(lambda x: str(x), encrypted_msg)))
        print("Decrypted msg: ")
        print(self.decrypt(encrypted_msg, private))
    ...
    def encrypt(self, msg_plaintext, package):
        # unpack key value pair
        e, n = package
        msg_ciphertext = [pow(ord(c), e, n) for c in msg_plaintext]
        return msg_ciphertext

    def decrypt(self, msg_ciphertext, package):
        d, n = package
