...
class DES:
    def __init__(self, plaintext, key):
        self.DESalgorythm(plaintext, key)

    def DESalgorythm(self, plaintext, key):
        ...
        # Encryption
        ciphertext = self.DESEncryption(key, plaintext, isPaddingRequired)

        # Decryption
        plaintext = self.DESDecryption(key, ciphertext, isPaddingRequired)
    ...
    def DESEncryption(self, key, text, padding):
        ...
        # Encryption
        ciphertext = self.DES1(text, key, padding, True)

        # Returning ciphertext
        return ciphertext

    def DESDecryption(self, key, text, padding):
        """Function for DES Decryption."""

        # Decryption
        plaintext = self.DES1(text, key, padding, False)
    ...
    def DES1(self, text, key, padding, isEncrypt):
        ...
        plaintext8byteBlocks = self.nSplit(text, 8)
        result = []
        ...
        for block in plaintext8byteBlocks:
            ...
            block = self.stringToBitArray(block)

            # Do the initial permutation
            block = self.permutation(block, initialPermutationMatrix)

            # Splitting block into two 4 byte (32 bit) sized blocks
            leftBlock, rightBlock = self.nSplit(block, 32)
            ...
            for i in range(16):
                ...
                expandedRightBlock = self.expand(rightBlock, expandMatrix)

                # Xor right block with appropriate key
                if isEncrypt == True:
                    # For encryption, starting from first key in normal order
                    temp = self.xor(keys[i], expandedRightBlock)
                elif isDecrypt == True:
                    # For decryption, starting from last key in reverse order
                    temp = self.xor(keys[15 - i], expandedRightBlock)
                # Sbox substitution Step
                temp = self.SboxSubstitution(temp)
                # Permutation Step
                temp = self.permutation(temp, eachRoundPermutationMatrix)
                # XOR Step with leftBlock
                temp = self.xor(leftBlock, temp)
            ...
            result += self.permutation(rightBlock + leftBlock, finalPermutationMatrix)

        # Converting bit array to string
        finalResult = self.bitArrayToString(result)
    ...
    def generateKeys(self, key):
        ...
        keys = []
        key = self.stringToBitArray(key)

        # Initial permutation on key
        key = self.permutation(key, keyPermutationMatrix1)

        # Split key in to (leftBlock->LEFT), (rightBlock->RIGHT)
        leftBlock, rightBlock = self.nSplit(key, 28)

        # 16 rounds of keys
        for i in range(16):
            # Do left shifting (different for different rounds)
            leftBlock, rightBlock = self.leftShift(leftBlock, rightBlock, SHIFT[i])
            # Merge them
            temp = leftBlock + rightBlock
            # Permutation on shifted key to get next key
            keys.append(self.permutation(temp, keyPermutationMatrix2))
    ...
    def SboxSubstitution(self, bitArray):
        ...
        for i in range(len(blocks)):
            ...
            binVal = self.binValue(sboxValue, 4)
            # Appending to result
    ...
    def stringToBitArray(self, text):
